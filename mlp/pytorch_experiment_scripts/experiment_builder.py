...
class ExperimentBuilder(nn.Module):
    ...
    def run_evaluation_iter(self, x, y):
        ...
        y_scal = np.argmax(y, axis=1)  # convert one hot encoded labels to single integer labels
        x, y = torch.Tensor(x).float().to(device=self.device), torch.Tensor(y_scal).long().to(
        ...
        _, predicted = torch.max(out.data, 1)  # get argmax of predictions
        accuracy = np.mean(list(predicted.eq(y.data).cpu()))  # compute accuracy
        return loss.data, accuracy, y_scal, predicted.tolist()
    ...
    def run_experiment(self):
        ...
        """
        total_losses = {"train_acc": [], "train_loss": [], "train_files":[], "train_labels":[], "val_acc": [],
                        "val_loss": [], "val_files":[], "val_labels":[], "val_pred_labels":[]}  # initialize a dict to keep the per-epoch metrics
        for i, epoch_idx in enumerate(range(self.starting_epoch, self.num_epochs)):
            epoch_start_time = time.time()
            current_epoch_losses = {"train_acc": [], "train_loss": [], "train_files":[], "train_labels":[], "val_acc": [], "val_loss": [], "val_files":[], "val_labels":[],  "val_pred_labels":[]}

            with tqdm.tqdm(total=self.train_data.num_batches) as pbar_train:  # create a progress bar for training
                for idx, (x, y, f_n_train) in enumerate(self.train_data):  # get data batches
                    loss, accuracy = self.run_train_iter(x=x, y=y)  # take a training iter step
                    current_epoch_losses["train_loss"].append(np.array(loss.cpu()))  # add current iter loss to the train loss list
            ...
            with tqdm.tqdm(total=self.val_data.num_batches) as pbar_val:  # create a progress bar for validation
                for x, y_val, f_n_val in self.val_data:  # get data batches
                    loss, accuracy, y_targets, predicted_labels = self.run_evaluation_iter(x=x, y=y_val)  # run a validation iter
                    current_epoch_losses["val_loss"].append(np.array(loss.cpu()))  # add current iter loss to val loss list.
                    current_epoch_losses["val_acc"].append(accuracy)  # add current iter acc to val acc lst.
                    current_epoch_losses["val_files"].append(f_n_val)
                    current_epoch_losses["val_labels"].append(y_targets)
                    current_epoch_losses["val_pred_labels"].append(predicted_labels)
            ...
            if val_mean_accuracy > self.best_val_model_acc:  # if current epoch's mean val acc is greater than the saved best val acc then
                ...
                total_losses["val_pred_labels"] = current_epoch_losses["val_pred_labels"]
                total_losses["train_files"] = current_epoch_losses["train_files"]
                total_losses["train_labels"] = current_epoch_losses["train_labels"]

            for key, value in current_epoch_losses.items():
                if key not in ["val_files", "val_labels", "val_preds", "val_pred_labels", "train_files", "train_labels", "test_files", "test_labels", "test_pred_labels"]:
        ...
        total_losses["val_files"] = [item for sublist in total_losses["val_files"] for item in sublist]
        total_losses["val_labels"] = [item for sublist in total_losses["val_labels"] for item in sublist]
        #total_losses["val_preds"] = [item for sublist in total_losses["val_files"] for item in sublist]
        total_losses["val_pred_labels"] = [item for sublist in total_losses["val_pred_labels"] for item in sublist]
        total_losses["train_files"] = [item for sublist in total_losses["train_files"] for item in sublist]
        total_losses["train_labels"] = [item for sublist in total_losses["train_labels"] for item in sublist]
        ...
        current_epoch_losses = {"test_acc": [], "test_loss": [], "test_files":[], "test_labels":[], "test_pred_labels":[]}  # initialize a statistics dict
        with tqdm.tqdm(total=self.test_data.num_batches) as pbar_test:  # ini a progress bar
            for x, y, f_name_test in self.test_data:  # sample batch
                loss, accuracy, y_targets, predicted_labels = self.run_evaluation_iter(x=x,
                                                          y=y)  # compute loss and accuracy by running an evaluation step
                current_epoch_losses["test_loss"].append(np.array(loss.cpu()))  # save test loss
                current_epoch_losses["test_acc"].append(accuracy)  # save test accuracy
                current_epoch_losses["test_files"].append(f_name_test)  # save test accuracy
                current_epoch_losses["test_labels"].append(y_targets)
                current_epoch_losses["test_pred_labels"].append(predicted_labels)
        ...
        test_losses = {key: [np.mean(value)] for key, value in
                       current_epoch_losses.items() if key not in ["test_files", "test_labels", "test_pred_labels"]}  # save test set metrics in dict format
