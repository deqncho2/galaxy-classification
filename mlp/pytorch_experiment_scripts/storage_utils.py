...
def save_statistics(experiment_log_dir, filename, stats_dict, current_epoch, save_full_dict=False):
    ...
    stats_dict_pruned = {k:v for (k, v) in stats_dict.items() if k not in ["val_files", "val_labels", "val_preds", "val_pred_labels", "train_files", "train_labels", "test_files", "test_labels", "test_pred_labels"]}
    ...
    with open(summary_filename, mode) as f:
        writer = csv.writer(f)
        if current_epoch == 0:
            writer.writerow(list(stats_dict_pruned.keys()))

        if save_full_dict:
            total_rows = len(list(stats_dict_pruned.values())[0])
            for idx in range(total_rows):
                row_to_add = [value[idx] for value in list(stats_dict_pruned.values())]
                writer.writerow(row_to_add)
        else:
            print(stats_dict_pruned)
            row_to_add = [value[current_epoch] for value in list(stats_dict_pruned.values())]
            writer.writerow(row_to_add)
