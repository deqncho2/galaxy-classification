...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        """
        output_width = inputs.shape[2] - self.kernels.shape[2] + 1
        output_height = inputs.shape[3] - self.kernels.shape[3] + 1
        outputs = np.empty((inputs.shape[0], self.num_output_channels, output_width, output_height))
        for k in range(self.num_output_channels):          # iterate over output feature map
            for i in range(inputs.shape[0]):               # iterate over batch
                outputs[i,k,:,:] = np.squeeze(signal.convolve(inputs[i,:,:,:], self.kernels[k,::-1,:,:], mode='valid', method='direct'))
        return (outputs.T + self.biases.reshape(-1,1)).T
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        for i in range(inputs.shape[0]):               # iterate over batch
            for j in range(self.num_input_channels):   # iterate over input feature map
                grads_wrt_inputs[i,j,:,:] = signal.convolve(grads_wrt_outputs[i,:,:,:], self.kernels[::-1,j,::-1,::-1], method='direct')[1,:,:]
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        output_width = inputs.shape[2] - grads_wrt_outputs.shape[2] + 1
        output_height = inputs.shape[3] - grads_wrt_outputs.shape[3] + 1
        grads_wrt_weights = np.empty((self.num_output_channels, self.num_input_channels, output_width, output_height))
        for k in range(self.num_output_channels):       # iterate over output feature map
            for j in range(self.num_input_channels):    # iterate over input feature map
                grads_wrt_weights[k,j,::-1,::-1] = np.squeeze(signal.correlate(inputs[:,j,:,:], grads_wrt_outputs[:,k,:,:], mode='valid', method='direct'))
        return [grads_wrt_weights, np.sum(np.sum(np.sum(grads_wrt_outputs, axis=3), axis=2), axis=0)]
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        outputs = np.empty((inputs.shape[0], inputs.shape[1], output_height, output_width))
        for y in range(output_height):
            for x in range(output_width):
                outputs[:,:,y,x] = np.max(inputs[:,:,y*self.stride:y*self.stride+self.size,x*self.stride:x*self.stride+self.size], axis=(2,3))
        return outputs        
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        expanded_grads_wrt_outputs = np.kron(grads_wrt_outputs, np.ones((1,1,2,2), dtype=grads_wrt_outputs.dtype))
        expanded_outputs = np.kron(outputs, np.ones((1,1,2,2), dtype=outputs.dtype))
        return expanded_grads_wrt_outputs*(expanded_outputs == inputs)
